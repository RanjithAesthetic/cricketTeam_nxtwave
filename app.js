...
app.post("/players/", async(request,response)=>{
    ...
    const api2 = `INSERT INTO cricket_team (player_name,jersey_number,role) VALUES ('${player_name}',${jersey_number},'${role}');`;
...
app.put("/players/:playerId/`", async(request,response)=>{
    ...
    const details = request.body;
    const {playerName,jerseyNumber,role} = details;    
    const api4 = `UPDATE cricket_team (player_name,jersey_number,role) VALUES ('${player_name}',${jersey_number},'${role}' WHERE player_id = ${playerId});`;
    await db.run(api4);
